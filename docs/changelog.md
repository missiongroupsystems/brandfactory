...
- **1.11.1** — 2026-07-30 — Stages 1–2 review remediation (blob content-type, colour XSS, undo delete, reorder route).
- **1.11.0** — 2026-07-30 — Brand research end to end (Stage 3A–3G). Migration 0005.
- **1.10.0** — 2026-07-29 — Brand assets end to end (Stage 2A–2F). Migration 0004.
- **1.9.0** — 2026-07-29 — Foundation: `website_url` + guideline `createdBy` provenance. Migration 0003.
- **1.8.0** — 2026-07-29 — Brand hub front-end mockup (data route + pure view; demo later demolished).
- **1.7.1** — 2026-07-29 — Mission Systems CI conformance pass.
- **1.7.0** — 2026-07-28 — Brand hub restructure: monogram, tiles, context rail.
- **1.6.0** — 2026-07-28 — Brand switcher in the app shell header.
- **1.5.1** — 2026-07-28 — Session token refresh (expired Supabase JWT 401s).
- **1.5.0** — 2026-07-28 — Brand context capture (conversation + promote-to-guidelines).
- **1.4.0** — 2026-07-25 — Brand hub mini-apps (context bar, tiles, template threads).
- **1.3.0** — 2026-07-22 — Reconcile with OSS upstream 0.8.1→0.9.1; restore `migrate.mjs`. 292 tests.
- **1.2.0** — 2026-07-20 — Mission Systems CI on `packages/web`: Satoshi, tokens, primitives.
- **1.1.0** — 2026-05-18 — Projects reachable + functional (create project on brand page).
- **1.0.0** — 2026-04-21 — Hosted-deploy baseline (Supabase, Fly, Vercel SPA). 238 tests.
- **0.9.1** — 2026-07-22 — Repo split: deploy layer out of OSS upstream; ISO timestamps; release notes for v7/v8.
- **0.9.0** — 2026-07-22 — Navigation redesign: workspace home, brand hub, switcher, breadcrumbs. 285 tests.
- **0.8.1** — 2026-07-22 — Fly deploy recipe recovered (`fly.toml`, Dockerfile, `migrate.mjs`).
- **0.8.0** — 2026-04-20 — Phase 8: `db:seed`, `.env.example`, CI, CORS gate. 234 tests.
- **0.7.4** — 2026-04-20 — Phase 7 Steps 12–16: canvas pane, agent events, shell polish. 223 tests.
- **0.7.3** — 2026-04-20 — Phase 7 Steps 7–11: workspaces/brands UI, brand editor, project split-screen.
- **0.7.2** — 2026-04-20 — Phase 7 Steps 3–6: router, auth, API client, realtime client.
- **0.7.1** — 2026-04-20 — Phase 7 Steps 0–2: frontend-facing routes + Vite/React scaffold. 167 tests.
- **0.7.0** — 2026-04-19 — Phase 6: agent SSE streaming end to end. 140 tests.
- **0.6.1** — 2026-04-19 — Pre-Phase-6 hardening: WS heartbeat, signature skew, db vitest. 120 tests.
- **0.6.0** — 2026-04-19 — Phase 5: `@brandfactory/agent` `streamResponse`. 106 tests.
- **0.5.1** — 2026-04-19 — Post-Phase-4 cleanup: tx helpers, LLM id single-source, settings validation.
- **0.5.0** — 2026-04-19 — Phase 4: bootable Hono server + six route modules. 83 tests.
- **0.4.1** — 2026-04-19 — Pre-Phase-4 cleanup: env exhaustiveness, type-aware ESLint, UUID regex.
- **0.4.0** — 2026-04-19 — Phase 3: adapter ports + `loadEnv` / `buildAdapters`. 31 tests.
...
**Creating a brand with a URL looked like research, then silence.** The symptom
was right; the causes were one layer further back: create never started a run,
the in-flight surface said almost nothing, and the deployment was still dark.
This release closes all three — the last of them by lighting production on
purpose, not by changing the code default.

Detail in
[`docs/completions/research-visibility-and-create-opt-in.md`](completions/research-visibility-and-create-opt-in.md).

### 1. Create did not start research

`NewBrandDialog` only `POST`ed the brand and navigated. Decision 1 of the locked
research onboarding doc had two entry points — the create-dialog checkbox and
the rail's re-run — and only the rail was ever wired. `useStartResearch`'s own
comment still said "and (later) the create dialog."

The checkbox is here now:
...
`useResearchConfig()` (long `staleTime`; only changes on redeploy).
`startResearchJob(brandId)` is extracted so dialog and rail share one POST.

### 2. In-flight was elapsed time on one page

The rail's `IN_PROGRESS` state was a single line of elapsed time, only on the
...
Cost and duration are single-sourced in `@brandfactory/shared`
(`RESEARCH_COST_ESTIMATE` ≈$0.40, `RESEARCH_DURATION_RANGE` 3–15 minutes —
measured at 3A/3G, not guessed). Presentation strings live in
`packages/web/src/lib/research-copy.ts`.

### 3. Enabling: documented in code, lit up in production

The **code** still defaults to dark — `RESEARCH_PROVIDER=none` in `EnvSchema`,
unset in `fly.toml` — so a fresh self-host never spends money by accident.
...
**Production light-up (same day, ops not code):** Fly app `brandfactory` now
has secrets `PERPLEXITY_API_KEY` and `RESEARCH_PROVIDER=perplexity` set via
`fly secrets set`; machines rolled healthy. The live UI therefore offers the
create checkbox and the rail action. Each run is still ~$0.40 and 3–15 minutes;
caps (`RESEARCH_MAX_ACTIVE_PER_WORKSPACE`, `RESEARCH_MAX_JOBS_PER_DAY`) still
fire above the spend line. The repo default remains `none` so the next clone
does not inherit production spend.

### What did not change (on purpose)

- No partial results while running (vendor contract).
- No silent auto-start on create without the checkbox.
- No cancel route (vendor bills for work already done).
- No migration — schema and job lifecycle are Stage 3's.
...
fly secrets list -a brandfactory  PERPLEXITY_API_KEY + RESEARCH_PROVIDER Deployed
```

885 → **897 (+12)**: three for `GET /research`, five for create-dialog research
paths, four for the header chip; rail expectation copy is an extended assertion.
Skips are live-Postgres. **No measured vendor run in this pass** — production
is now capable of one; the first real post-1.13 run is the live verification.
...
### The item, and the three things it copies

`New brand…` sits between the brand list and `All brands`, mirroring
`WorkspaceSwitcher` rather than inventing a second convention:

- **Create above the way out**, as in the workspace menu.
- **Opening is deferred a tick.** A dialog opened inside `onSelect` leaves the
  menu's focus scope fighting the dialog's — the reason `deferUntilMenuClosed`
...
### The dialog moved, and that is what earned the behaviour change

`NewBrandDialog` lived inside `routes/workspaces.$wsId.index.tsx`. It moves to
`components/NewBrandDialog.tsx` and takes the controlled/uncontrolled + `trigger`
shape `NewWorkspaceDialog` already had for exactly this situation — the route
passes its `+ Brand` button in as `trigger`, the switcher passes `open` and no
trigger, because a menu item is the opener and a second button in the header
would be wrong.

One behaviour changed with it: the inline version cleared its fields **only on
success**. Survivable on a page you had to navigate to; wrong on an affordance
reachable from every page in a brand, where a cancelled attempt handed the next
one a half-typed name — and a website error about a field that was now empty. It
now clears on close, which is the convention `NewWorkspaceDialog` already
documented.
...
pnpm typecheck                    10/10 workspaces
pnpm lint / format:check          clean
pnpm test                         885 passed | 47 skipped (102 files)
```

921 → **932 (+11)**: nine for the extracted dialog, which had **none** while it
sat in the route, and two for the switcher. The nine include 1A's stored-XSS gate
from the form's side — `javascript:alert(1)` is reported inline and never
posted — and the clear-on-close above.

**The 47 skips are all live-Postgres files**, and are this environment rather
than this change: there is no Docker and no `.env`, so `DATABASE_URL` is unset.
The change is `packages/web` only — no migration, no route, no server change —
so nothing in it is reachable from those files.

**No live pass**, for the same reason: without Postgres the app cannot boot. What
went unobserved is presentation only — where the item sits once the list scrolls
past `max-h-80`, and whether the separator still reads correctly with two items
below it instead of one.
