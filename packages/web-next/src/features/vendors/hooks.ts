...
export function useVendorPages(params: Parameters<typeof vendorService.list>[0] = {}) {
  return useCursorPages<VendorListItem>(SCOPES.vendors, params ?? {}, (cursor) =>
 ...
 * is why it is the most-imported thing the vendors feature owns. An id absent from
 ...
 *
 * On `SCOPES.vendor`, which `useVendorMutations` already invalidates on update,
...
export function useVendor(id: string | undefined) {
  return useSWR(id ? [SCOPES.vendor, id] : null, () => vendorService.get(id!));
}
...
export function useVendorMutations() {
  ...
  const create = React.useCallback(
    async (data: VendorCreate) => {
      ...
      await invalidate(SCOPES.vendors, SCOPES.contacts);
      return created;
    },
    [invalidate],
  );
  ...
  const update = React.useCallback(
    async (id: string, data: VendorUpdate) => {
      ...
      await invalidate(SCOPES.vendors, SCOPES.vendor);
      return updated;
    },
    [invalidate],
  );
  ...
  const replaceContacts = React.useCallback(
    async (id: string, contacts: VendorContactInput[]) => {
      ...
      await invalidate(SCOPES.vendors, SCOPES.vendor, SCOPES.contacts);
      return updated;
    },
    [invalidate],
  );
  ...
  const remove = React.useCallback(
    async (id: string) => {
      ...
      await invalidate(SCOPES.vendors, SCOPES.vendor, SCOPES.contracts, SCOPES.contacts);
    },
    [invalidate],
  );
